...
pub fn build_chat_payload(
    ...
    DuckChatRequest {
        ...
        can_use_tools: true,
        reasoning_effort: if is_image_gen {
            "low".to_string()
        } else {
            max_reasoning_effort_for_model(duck_model).to_string()
        },
...
/// Runtime and test guard enforcing that a chat request payload is strictly in Duck.ai reasoning mode.
pub fn guard_reasoning_mode(payload: &DuckChatRequest) -> Result<(), &'static str> {
    if !payload.is_reasoning_mode() {
        return Err("ReasoningModeGuardViolation: payload reasoning_effort must be 'low' or 'medium' (reasoning mode)");
    }
    Ok(())
}

/// Test guard assertion that panics if the payload is not strictly in Duck.ai reasoning mode.
pub fn assert_reasoning_mode(payload: &DuckChatRequest) {
    if let Err(e) = guard_reasoning_mode(payload) {
        panic!("{}", e);
    }
}
...
mod tests {
    ...
    fn test_build_chat_payload_basic() {
        ...
        assert_eq!(payload.reasoning_effort, REASONING_EFFORT_REASONING);
        assert!(payload.is_reasoning_mode());
        assert_reasoning_mode(&payload);
    }

    #[test]
    fn test_payload_always_in_reasoning_mode() {
        let keypair = EphemeralKeypair::generate().unwrap();
        let messages = vec![DuckChatMessage {
            role: "user".to_string(),
            content: "solve complex math problem".to_string(),
        }];

        let payload = build_chat_payload("gpt-5.6-luna", messages, &keypair, false, "conv-reason-1");
    ...
    #[test]
    fn test_guard_reasoning_mode_accepts_valid() {
        let keypair = EphemeralKeypair::generate().unwrap();
        let payload = build_chat_payload("gpt-5.4-mini", vec![], &keypair, false, "conv-valid");
    ...
    #[test]
    fn test_guard_reasoning_mode_rejects_none() {
        let keypair = EphemeralKeypair::generate().unwrap();
        let mut payload = build_chat_payload("gpt-5.6-luna", vec![], &keypair, false, "conv-violation");
    ...
    fn test_guard_reasoning_mode_panics_on_violation() {
        let keypair = EphemeralKeypair::generate().unwrap();
        let mut payload = build_chat_payload("gpt-5.6-luna", vec![], &keypair, false, "conv-panic");
        payload.reasoning_effort = "none".to_string();
        assert_reasoning_mode(&payload);
    }

    #[test]
    fn test_reasoning_mode_across_chat_models() {
        let keypair = EphemeralKeypair::generate().unwrap();
        let models = vec![
            ("gpt-5.6-luna", "low"),
            ("gpt-5.4-mini", "medium"),
        ...
        for (model, expected_effort) in models {
            let payload = build_chat_payload(
                model,
                vec![DuckChatMessage { role: "user".to_string(), content: "test".to_string() }],
                &keypair,
                false,
                "conv-multi",
            );
            assert_eq!(
                ...
                "Model {} must be in max reasoning mode ('{}')", model, expected_effort
            );
            assert!(payload.is_reasoning_mode());
            assert_reasoning_mode(&payload);
        }
