...
pub fn sec_ch_ua_for_ua(ua: &str) -> String {
    if ua.contains("Edg/") {
        if let Some(pos) = ua.find("Edg/") {
            let ver = ua[pos + 4..].split('.').next().unwrap_or("134");
            format!(r#""Not(A:Brand";v="99", "Microsoft Edge";v="{ver}", "Chromium";v="{ver}""#)
        } else {
            r#""Not(A:Brand";v="99", "Microsoft Edge";v="134", "Chromium";v="134""#.to_string()
        }
    } else if ua.contains("Firefox") {
        r#""Not A(Brand";v="99", "Firefox";v="136""#.to_string()
    } else if ua.contains("Safari") && !ua.contains("Chrome") {
        r#""Not A(Brand";v="99", "Safari";v="18""#.to_string()
    } else {
        if let Some(pos) = ua.find("Chrome/") {
            let ver = ua[pos + 7..].split('.').next().unwrap_or("134");
            format!(r#""Not(A:Brand";v="99", "Google Chrome";v="{ver}", "Chromium";v="{ver}""#)
        } else {
            r#""Not(A:Brand";v="99", "Google Chrome";v="134", "Chromium";v="134""#.to_string()
        }
    }
}
...
pub struct VirtualUser {
    ...
    pub global_rate_limited_until: Arc<RwLock<Option<chrono::DateTime<chrono::Utc>>>>,
...
impl VirtualUser {
    pub fn new(id: &str, ua_index: usize) -> Self {
        let ua = if ua_index < USER_AGENTS.len() {
            USER_AGENTS[ua_index].to_string()
        } else {
            let platforms = [
            ...
            let plat = platforms[ua_index % platforms.len()];
            let chrome_vers = [130, 131, 132, 133, 136];
            let chrome_ver = chrome_vers[ua_index % chrome_vers.len()];
            let minor = (ua_index * 19 + 7) % 50;
            let build = (ua_index * 37 + 13) % 90;
            format!("Mozilla/5.0 ({plat}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{chrome_ver}.0.{minor}.{build} Safari/537.36")
        };
        ...
        Self {
            ...
            global_rate_limited_until: Arc::new(RwLock::new(None)),
            successful_requests: Arc::new(AtomicU64::new(0)),
            rate_limited_requests: Arc::new(AtomicU64::new(0)),
        }
    }

    /// Returns a clone of the current HTTP client with shared session cookies.
    pub async fn client(&self) -> reqwest::Client {
        self.http.read().await.clone()
    }

    /// Resets the virtual user's HTTP client with a fresh cookie jar and clears cached session tokens.
    pub async fn reset_session_and_cookies(&self) {
        let mut headers = build_browser_headers_for_ua(&self.user_agent);
        headers.remove("host");
        ...
        {
            let mut client_guard = self.http.write().await;
            *client_guard = new_http;
        }
    ...
    /// Checks if this virtual user is currently blocked by a 429 rate limit on a specific model or globally.
    pub async fn is_model_rate_limited(&self, model: &str) -> bool {
        {
            let global = self.global_rate_limited_until.read().await;
    ...
    pub async fn mark_model_rate_limited(&self, model: &str, reset_at: Option<chrono::DateTime<chrono::Utc>>) {
        ...
        let reset = match reset_at {
            Some(r) if r > now => r,
            _ => now + chrono::Duration::minutes(15),
        };
        let mut limits = self.rate_limited_until.write().await;
        limits.insert(model.to_string(), reset);
    }

    /// Marks this virtual user as globally rate-limited across all models until the specified reset time.
    pub async fn mark_global_rate_limited(&self, reset_at: Option<chrono::DateTime<chrono::Utc>>) {
        self.rate_limited_requests.fetch_add(1, Ordering::Relaxed);
        let now = chrono::Utc::now();
        let reset = match reset_at {
            Some(r) if r > now => r,
        ...
        let mut global = self.global_rate_limited_until.write().await;
        *global = Some(reset);
    }
    ...
    pub async fn warm(&self, upstream_base_url: &str, fe_version: &Arc<RwLock<String>>, journey_id: &str) {
        ...
        if let Ok(resp) = client.get(upstream_base_url).send().await {
        ...
        let auth_url = format!("{}/duckchat/v1/auth/token", upstream_base_url.trim_end_matches('/'));
        let _ = client.get(&auth_url)
            .header("user-agent", &self.user_agent)
            .header("referer", format!("{}/", upstream_base_url.trim_end_matches('/')))
            .header("origin", upstream_base_url.trim_end_matches('/'))
            .send().await;
    ...
    pub async fn force_rewarm(&self, _journey_id: &str) {
        self.reset_session_and_cookies().await;
    }
    ...
    pub async fn fetch_raw_status_challenge(
        ...
        if body_text.contains("\"isBlocked\":true") || body_text.contains("\"status\":\"1\"") || body_text.contains("fixedCostWindowUsage") {
            let reset_at = serde_json::from_str::<serde_json::Value>(&body_text)
                .ok()
                .and_then(|v| {
            ...
            self.mark_global_rate_limited(reset_at).await;
            return Err(AppError::upstream_rate_limit(
                format!("Duck.ai status rate limit (isBlocked) for VirtualUser '{}': {}", self.id, body_text),
                Some(4),
            ));
        }

        Err(AppError::bad_gateway(format!("No x-vqd challenge returned in status response: {}", body_text)))
    }
...
impl VirtualUserPool {
    ...
    pub async fn select_user_for_model(&self, model: &str, preferred_user_id: Option<&str>) -> VirtualUser {
        ...
        for offset in 0..total {
            ...
            if !u.is_model_rate_limited(model).await {
                self.active_index.store((idx + 1) % total, Ordering::Relaxed);
                return u.clone();
            }
        }

        // If pool already has 32 or more users, replace the next user with a fresh one instead of unboundedly allocating
        if total >= 32 {
            let replace_idx = (start + 1) % total;
            let new_num = self.dynamic_counter.fetch_add(1, Ordering::Relaxed);
            let fresh_user = VirtualUser::new(&format!("vu-dyn-{}", new_num), new_num);
            drop(users);
            let mut write_guard = self.users.write().await;
            write_guard[replace_idx] = fresh_user.clone();
            self.active_index.store(replace_idx, Ordering::Relaxed);
            return fresh_user;
        }
        drop(users);
    ...
    /// Rotates to the next virtual user on 418 challenge rejection, cooling down the failing user for 2 minutes.
    pub async fn rotate_on_challenge(&self, current_user_id: &str, model: &str) -> VirtualUser {
        {
            let users = self.users.read().await;
            if let Some(u) = users.iter().find(|u| u.id == current_user_id) {
                u.mark_model_rate_limited(model, Some(chrono::Utc::now() + chrono::Duration::minutes(2))).await;
            }
        }
        self.active_index.fetch_add(1, Ordering::Relaxed);
        self.select_user_for_model(model, None).await
