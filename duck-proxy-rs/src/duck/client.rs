...
impl DuckClient {
    ...
    pub async fn send_chat_request_cascade(
        ...
        for (idx, candidate_model) in fallback_chain.iter().enumerate() {
            ...
            for vu_attempt in 0..max_vu_attempts {
                ...
                current_vu.warm(&self.upstream_base_url, &self.fe_version, &session.journey_id).await;
                ...
                if !is_image_gen {
                    crate::duck::payload::assert_reasoning_mode(&payload);
                }
                ...
                let client = current_vu.client().await;
                ...
                match request.json(&payload).send().await {
                    Ok(resp) => {
                        if resp.status() == reqwest::StatusCode::TOO_MANY_REQUESTS {
                            ...
                            let backoff_ms = if is_image_gen { 500 } else { 250 };
                            tokio::time::sleep(tokio::time::Duration::from_millis(backoff_ms)).await;
                            continue;
                        }

                        if resp.status().as_u16() == 418 {
                            let body_418 = resp.text().await.unwrap_or_default();
                            tracing::warn!(
                                "Duck.ai challenge rejected (418) for VirtualUser '{}' on model '{}', body: '{}', resetting session & cookies...",
                                current_vu.id,
                                candidate_model,
                                body_418
                            );

                            current_vu.reset_session_and_cookies().await;
                            ...
                            let fresh_vqd = match current_vu.get_solved_challenge_header(
                                ...
                                Ok(v) => v,
                                Err(e) => {
                                    tracing::warn!("Failed to solve challenge on 418 retry: {:?}", e);
                                    solved_vqd.clone()
                                }
                            };

                            let retry_client = current_vu.client().await;
                            ...
                            if !is_image_gen {
                                crate::duck::payload::assert_reasoning_mode(&retry_payload);
                            }

                            match retry_req.json(&retry_payload).send().await {
                                Ok(retry_resp) => {
                                    if retry_resp.status().is_success() {
                                        ...
                                        tracing::warn!(
                                            "Retry after 418 on model '{}' returned HTTP {}: '{}'",
                                            candidate_model,
                                            retry_status,
                                            retry_body
                                        );
                                        if retry_status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                                            current_vu = self.user_pool.rotate_on_rate_limit(&current_vu.id, candidate_model, None).await;
                                            tracing::warn!("🔄 Switched to VirtualUser '{}' after 429 on 418 retry for model '{}'", current_vu.id, candidate_model);
                                            last_err = Some(AppError::upstream_rate_limit(
                                                format!("Duck.ai model '{}' rate limited (HTTP 429) on 418 retry", candidate_model),
                                                Some(4),
                                            ));
                                            let backoff_ms = if is_image_gen { 500 } else { 250 };
                                            tokio::time::sleep(tokio::time::Duration::from_millis(backoff_ms)).await;
                                ...
                                Err(e) => {
                                    tracing::warn!("Retry after 418 on model '{}' failed: {:?}", candidate_model, e);
                                }
                            }

                            // Rotate to next virtual user on challenge failure
                            current_vu = self.user_pool.rotate_on_challenge(&current_vu.id, candidate_model).await;
    ...
    pub async fn send_chat_request_with_vqd(
        ...
        for attempt in 0..MAX_RETRIES {
            ...
            let fe_version = self.fe_version.read().await.clone();
            let client = active_user.client().await;
            let mut request = client
                .post(&url)
            ...
            if resp.status().as_u16() == 418 {
                tracing::warn!("Duck.ai challenge rejected (418) for model '{}', resetting session & cookies...", model);
                active_user.reset_session_and_cookies().await;
    ...
    /// Sends an audio binary to Duck.ai's dictation endpoint /duckchat/v1/dictation.
    pub async fn send_dictation_request(
        &self,
        audio_data: Vec<u8>,
        content_type: &str,
    ) -> Result<String, AppError> {
        let url = format!("{}/duckchat/v1/dictation", self.upstream_base_url);
        let model = "gpt-5.6-luna";
        let active_user = self.user_pool.select_user_for_model(model, None).await;
        let session = active_user.get_or_create_session(model).await;

        let solved_vqd = active_user.get_solved_challenge_header(
            &self.upstream_base_url,
            model,
            &session.journey_id,
            &self.v8_actor,
        ...
        let mut request = client
            ...
            .header("x-duckai-chat-model", model);

        for (key, value) in active_user.generate_telemetry_headers(&session.journey_id, &fe_version) {
            request = request.header(&key, &value);
        }

        let resp = request.body(audio_data).send().await?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            return Err(AppError::bad_gateway(format!("Duck.ai dictation returned HTTP {}: {}", status, body)));
        }

        let json: serde_json::Value = resp.json().await.map_err(|e| AppError::bad_gateway(format!("Invalid dictation JSON response: {}", e)))?;
        let text = json.get("text").and_then(|v| v.as_str()).unwrap_or_default().to_string();
        Ok(text)
    }
