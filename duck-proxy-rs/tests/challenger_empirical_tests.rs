...
use duck_proxy_rs::api::chat::{
    build_image_write_tool_call, derive_image_filename, derive_image_slug, is_image_generation_intent,
...
fn test_complex_bracket_prompt_preservation_and_filename_derivation() {
    ...
    for (prompt, expected_filename) in complex_prompts {
        let expected_slug = expected_filename.trim_end_matches(".png");
        // 1. Check slug and filename derivation
        let slug = derive_image_slug(prompt);
        assert_eq!(
            slug, expected_slug,
            "Failed deriving slug for prompt: '{}', got: '{}'",
            prompt, slug
        );
        let fname = derive_image_filename(prompt);
        assert!(
            fname.starts_with(&format!("{}_", expected_slug)),
            "Filename '{}' should start with '{}_'", fname, expected_slug
        );
