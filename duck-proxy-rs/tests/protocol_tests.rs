...
fn test_derive_image_filename() {
    use duck_proxy_rs::api::chat::{derive_image_filename, derive_image_slug};

    assert_eq!(derive_image_slug("can u gen img of a horse"), "horse");
    assert_eq!(derive_image_slug("can u gen img of a knight and add your base url on top of it as small icon"), "knight_base_url");
    assert_eq!(derive_image_slug("generate a picture of futuristic cyberpunk cat"), "futuristic_cyberpunk_cat");
    assert_eq!(derive_image_slug(""), "image");

    let fname = derive_image_filename("can u gen img of a horse");
    assert!(fname.starts_with("horse_"), "Filename '{}' should start with 'horse_'", fname);
    assert!(fname.ends_with(".png"), "Filename '{}' should end with '.png'", fname);

    // Verify 12-digit random suffix and date/time format: {slug}_{YYYYMMDD}_{HHMMSS}_{12_digits}.png
    let parts: Vec<&str> = fname.trim_end_matches(".png").split('_').collect();
    assert!(parts.len() >= 4, "Filename parts should include slug, date, time, and 12-digit random number");
    let rand_part = parts.last().unwrap();
    assert_eq!(rand_part.len(), 12, "Random suffix should be 12 digits: {}", rand_part);
    assert!(rand_part.chars().all(|c| c.is_ascii_digit()), "Random suffix should only contain digits: {}", rand_part);
}
...
fn test_prompt_preservation_with_brackets() {
    ...
    assert_eq!(derive_image_slug(prompt_with_brackets), "cyberpunk_knight_neon");
    let filename = derive_image_filename(prompt_with_brackets);
    assert!(filename.starts_with("cyberpunk_knight_neon_"));
...
fn test_payload_retains_image_gen_flag() {
    ...
    // With is_image_gen = false (Always in reasoning mode)
    ...
    assert_eq!(text_payload.reasoning_effort, "low");
    assert!(text_payload.is_reasoning_mode());
    duck_proxy_rs::duck::payload::assert_reasoning_mode(&text_payload);
}
